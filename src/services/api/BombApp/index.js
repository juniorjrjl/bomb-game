...
export default BombService = {
    ...
    startCountDown: ({setSeconds, setMinutes, setHours, setStarted, diffTime, setIntervalId, intervalId, navigation}) =>{
    ...
    bombActivationTogether: ({question, pin, hours, minutes, seconds, setMessage, setStarted, setPin, handleStartBomb, setAnswer}) =>{
        ...
        if (pin.join("").length !== 3){
            setMessage("a senha deve ter 3 dígitos")
        }
        ...
        if (!timeIsSet){
            setMessage("Timer inválido, informe um tempo")
        }
    },

    bombDisarmTogether: ({pin, answer, setStarted, intervalId, setPin, setAnswer, navigation}) =>{
        if (pin.join("") === answer){
            clearInterval(intervalId)
            setStarted(false)
            navigation.navigate("Disarmed")
